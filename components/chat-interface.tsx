...
export default function ChatInterface() {
  ...
  const { welcomeMessage, buttonText, placeholderText, typingIndicator } = uiConfig.chat
      ...
      <div className="flex-1 overflow-y-auto mb-4 space-y-4">
        {messages.length === 0 && (
          <Card className="p-6 bg-card border-2 border-secondary/20">
            <h2 className="text-xl font-semibold mb-4 text-foreground">{welcomeMessage.title}</h2>
            <p className="text-muted-foreground mb-4 leading-relaxed">
              {welcomeMessage.introduction}
            </p>
            <ul className="space-y-2 text-muted-foreground">
              {welcomeMessage.capabilities.map((capability, index) => (
          ...
          <div className="flex justify-start">
            <Card className="max-w-[80%] p-4 bg-card text-card-foreground border-primary/20">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground">{typingIndicator}</span>
              </div>
            </Card>
          </div>
        )}
